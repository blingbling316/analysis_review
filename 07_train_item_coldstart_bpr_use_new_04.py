...
# ============================================================
# 显存友好的 item-cold-start 版本
# 1) 复用 04 的 item cold-start split
# 2) 使用 04 导出的 item-cold-start aligned 特征
# 3) minibatch 子图训练，避免 full-graph OOM
# 4) 训练目标：真正的 user-item BPR
# 5) 评估目标：只在 cold items 上评估
# ============================================================
...
class Config:
    ...
    # ---------- 输出 ----------
    save_dir: str = "outputs_item_coldstart_minibatch"
    best_model_name: str = "best_model_item_coldstart_minibatch.pth"
...
def prepare_data(num_items: int):
    ...
    print(
        f"[INFO] Interactions by split | train={len(train_interactions):,} | val-cold={len(val_interactions):,} | test-cold={len(test_interactions):,}"
    )
    ...
    val_support_ratio = len(valid_val_users) / max(len(val_user_items), 1)
    test_support_ratio = len(valid_test_users) / max(len(test_user_items), 1)
    print(f"[INFO] Valid user coverage | val={val_support_ratio:.4f} | test={test_support_ratio:.4f}")
...
def main():
    ...
    print("[INFO] 07 is now reusing the same cold-item split as 04.")
    ...
    hist_path = os.path.join(cfg.save_dir, "train_history_item_coldstart_minibatch.csv")
